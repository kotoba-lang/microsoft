(ns microsoft.app-test
  (:require [cljs.test :refer [deftest is testing use-fixtures]]
            [re-frame.core :as rf]
            [re-frame.db :as rf-db]
            [microsoft.app :as app]))
...
(deftest initialize-db-sets-defaults
  (testing ":initialize-db populates every fact the Svelte scaffold held"
    ...
    (is (= "Microsoft M1cr5nd5" @(rf/subscribe [:app/title])))
    (is (= "etzhayyim-wasm-microsoft-m1cr5nd5" @(rf/subscribe [:app/name])))
    (is (= "etzhayyim-project-microsoft" @(rf/subscribe [:app/project])))
    ...
    (is (= "cljs/src/microsoft/app.cljs" @(rf/subscribe [:app/relative-path])))))
