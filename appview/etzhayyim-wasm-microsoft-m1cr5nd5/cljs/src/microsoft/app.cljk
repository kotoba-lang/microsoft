(ns microsoft.app
  "etzhayyim-wasm-microsoft-m1cr5nd5 appview — reagent + re-frame, view built from
  ...
  - `:app/xrpc?` stays `true` — this Worker's `main` (`src/app.ts`, this
    repo's Microsoft 365 write facade) registers
    `com.etzhayyim.apps.microsoft.{sendMail,sendDraft,listInbox,
    batchMoveMessages,listDrafts}` via `createWorkerExport` from
    `@etzhayyim/kotodama-host-sdk`, unrelated to this migration and
    untouched by it. The SvelteKit route this page used to describe
    (`svelte/src/routes/xrpc/[...path]/+server.ts`) was a *separate*,
    generic proxy to `AGENTGATEWAY_MCP_ROUTER_URL` — moved, not deleted,
    to `src/xrpc-proxy.ts` (see that file's header for how it differs
    from `src/app.ts`, not merely which upstream it calls) and NOT wired
    into this migration's `wrangler.jsonc` `main`.

  A fact this migration did NOT change and is flagging, not fixing:
  `src/app.ts` does not call `env.ASSETS.fetch(...)` anywhere in its
  handler (no `ASSETS` binding is referenced at all — verified by
  grepping the file). `wrangler.jsonc`'s `assets.directory` (now this
  build's `public/`) therefore has no in-Worker fallback route serving
  it; whether/how to wire that up is a pre-existing, separate product
  decision — see this migration's `wrangler.jsonc` comment header.
  ...
  components or ext-rules change) with:
    ...
    (spit \"public/index.html\"
          (page/->page {:title \"etzhayyim-project-microsoft\"
                         :lang \"ja\"
                         :description \"microsoft — etzhayyim Microsoft 365 Send appview facade status page (reagent + re-frame + jp-go-dds).\"
                         :css (slurp (io/resource \"jp_go_dds/dds.css\"))}
                        [:div {:id \"app\"} \"etzhayyim-project-microsoft loading…\"]
...
(def default-db
  {:app/title "Microsoft M1cr5nd5"
   :app/project "etzhayyim-project-microsoft"
   :app/name "etzhayyim-wasm-microsoft-m1cr5nd5"
   :app/kind "appview"
   :app/route-count 0
   :app/routes []
   :app/vars []
   :app/xrpc? true
   :app/relative-path "cljs/src/microsoft/app.cljs"})
